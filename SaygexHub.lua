...
Window = loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))():CreateWindow({
    ...
    ["MinimizeKey"] = Enum.KeyCode.End
        ...
        require(game:GetService("ReplicatedStorage").Notification).new("<Color=White>SAYGEX<Color=/>"):Display()
...
v1455.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1
	or input.UserInputType == Enum.UserInputType.Touch then
		dragging = true
		dragStart = input.Position
		startPos = v1455.Position
...
v1455.InputChanged:Connect(function(input)
	if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement
	or input.UserInputType == Enum.UserInputType.Touch) then
		local delta = input.Position - dragStart
		v1455.Position = UDim2.new(
			startPos.X.Scale,
...
local v94 = game.PlaceId
if ({
    [2753915549] = true,
    [4442272183] = true,
    [7449423635] = true
})[v94] then
    if v94 == 2753915549 then
        World1 = true
    elseif v94 == 4442272183 then
        World2 = true
    elseif v94 == 7449423635 then
        World3 = true
    end
else
...
Dropdown = Stats:AddDropdown("Dropdown", {
    ["Title"] = "Auto Stats",
    ["Values"] = {
        "Melee",
        "Defense",
...
if World1 then
    Dropdown = Isl:AddDropdown("Dropdown", {
        ["Title"] = "Select Island",
        ["Values"] = {
            "WindMill",
            "Marine",
...
spawn(function()
    while task.wait(0.2) do
        if _G.AutoNear and getgenv().Auto_Dungeon then
            pcall(function()
                    ...
                    and npc.Humanoid.Health > 0 then
                        ...
                        if distance >= 100 and distance <= 200 then
                            ...
                            currentTween = TweenService:Create(
                                HRP,
                                TweenInfo.new(
                                    0.6, -- TỐC ĐỘ BÌNH THƯỜNG
                                    Enum.EasingStyle.Linear
                                ),
                                {
                                    CFrame = npcHRP.CFrame * CFrame.new(0, 0, 4)
                                }
                            )
                            ...
                            break -- chỉ tween 1 NPC tại 1 thời điểm
                        end
                    end
                end
            end)
...
-- ======================
-- AUTO TP ĐẾN ĐẢO RAID
-- ======================
spawn(function()
    while task.wait(1) do
        if getgenv().Auto_Dungeon 
        and game.Players.LocalPlayer.PlayerGui.Main.TopHUDList.RaidTimer.Visible ~= false then
            for _, island in ipairs({
                ...
                local loc = game:GetService("Workspace")._WorldOrigin.Locations:FindFirstChild(island)
                if loc then
                    topos(loc.CFrame * CFrame.new(0,70,100))
                    break
                end
            end
        end
...
spawn(function()
    while task.wait(1) do
        pcall(function()
            if getgenv().Auto_Dungeon
            and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false
            and (game.Players.LocalPlayer.Backpack:FindFirstChild("Special Microchip")
            or game.Players.LocalPlayer.Character:FindFirstChild("Special Microchip")) then
                if World2 then
                    fireclickdetector(
                        game.Workspace.Map.CircleIsland.RaidSummon2.Button.Main.ClickDetector
                    )
                elseif World3 then
                    fireclickdetector(
                        game.Workspace.Map["Boat Castle"].RaidSummon2.Button.Main.ClickDetector
                    )
                end
            end
        end)
...
-- ======================
-- FUNCTION 2: AUTO NEXT ISLAND (KHÔNG KILL NPC)
-- ======================
_G.AutoNextIsland = false
_G.AutoNear = false
        ...
        if _G.AutoNextIsland then
            pcall(function()
                ...
                if Char and Char:FindFirstChild("HumanoidRootPart") then
                    local HRP = Char.HumanoidRootPart
                    local Locations = game.Workspace._WorldOrigin.Locations
                    local Pos = HRP.Position

                    -- Reset khi về tâm raid
                    ...
                    if Locations:FindFirstChild("Island 2") and not Visited["Island 2"] then
                        Tween(Locations["Island 2"].CFrame)
                        Visited["Island 2"] = true
                        task.wait(0.5)

                    elseif Locations:FindFirstChild("Island 3") and not Visited["Island 3"] then
                        Tween(Locations["Island 3"].CFrame)
                        Visited["Island 3"] = true
                        task.wait(0.5)

                    elseif Locations:FindFirstChild("Island 4") and not Visited["Island 4"] then
                        Tween(Locations["Island 4"].CFrame)
                        Visited["Island 4"] = true
                        task.wait(0.5)

                    elseif Locations:FindFirstChild("Island 5") and not Visited["Island 5"] then
                        Tween(Locations["Island 5"].CFrame)
                        Visited["Island 5"] = true
                        task.wait(0.5)
                    end
                end
            end)
...
v1455.MouseButton1Down:Connect(function()
	...
	-- END key DOWN
	vu1457:SendKeyEvent(true, Enum.KeyCode.End, false, game)
end)

v1455.MouseButton1Up:Connect(function()
	if not holding then return end
	holding = false

	-- END key UP
	vu1457:SendKeyEvent(false, Enum.KeyCode.End, false, game)
end)

v1455.MouseLeave:Connect(function()
	if holding then
		holding = false
		vu1457:SendKeyEvent(false, Enum.KeyCode.End, false, game)
	end
